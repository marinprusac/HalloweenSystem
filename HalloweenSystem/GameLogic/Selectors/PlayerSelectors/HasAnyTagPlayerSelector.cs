...
public class HasAnyTagPlayerSelector(ISelector<Tag> tagSelector, ISelector<Player>? playerSelector = null)
	: ISelector<Player>, IParser<HasAnyTagPlayerSelector>
{
	...
	public IEnumerable<Player> Evaluate(Context context)
	{
		...
		return players.Where(player => tags.Any(tag => player.GetQueriedTag(tag, out _)));
	}

	public static HasAnyTagPlayerSelector Parse(XmlNode node)
	{
		...
		return new HasAnyTagPlayerSelector(tagSelector, playerSelector);
	}
}